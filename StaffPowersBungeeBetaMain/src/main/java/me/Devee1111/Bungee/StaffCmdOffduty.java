...
public class StaffCmdOffduty extends Command {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(StaffSql.onDuty(pp.getUniqueId().toString()) == false) {
			TextComponent tosend = new TextComponent(inst.color(inst.createMessage("break.off-duty.nothing-changed").replace("%player%", pp.getName())));
			pp.sendMessage(tosend);
			return;
		}
		
		
		StaffSql.setDuty(pp.getUniqueId().toString(), false);
		
		//Sending to staff about the duty change
		pp.sendMessage(new TextComponent(inst.color(inst.createMessage("break.off-duty.to-player").replace("%player%", pp.getName()))));
		
		TextComponent tosend = new TextComponent(inst.color(inst.createMessage("break.off-duty.to-staff").replace("%player%", pp.getName())));
